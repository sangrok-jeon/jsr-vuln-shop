...
4. 포인트 사용뿐 아니라 주문 처리, 비밀번호 변경, 정보 수정 등 모든 상태 변경 엔드포인트에 동일한 CSRF 검증 정책을 확장 적용한다.
...
본 문서의 대응 증적은 `/point/use`를 중심으로 CSRF 토큰 검증을 적용한 사례다. 실제 서비스에서는 동일한 기준을 주문 처리, 비밀번호 변경, 기본 정보 수정 등 다른 상태 변경 기능에도 일관되게 확장하는 것이 바람직하다.
