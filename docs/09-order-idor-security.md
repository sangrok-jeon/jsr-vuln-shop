...
## 개요
주문 상세 조회 기능에서 확인한 `Broken Access Control / IDOR` 취약점과 그 대응 방안을 정리한다.

주문 내역 화면에서는 로그인한 사용자의 주문 목록만 표시되지만, 주문 상세 조회는 `orderId` 파라미터만으로 처리된다. 그 결과 다른 사용자의 주문번호를 추측하거나 변조하면 타인의 주문 상세 정보가 그대로 노출된다.
...
| 구분 | 취약점 | 설명 |
| --- | --- | --- |
| 주요 취약점 | Broken Access Control / IDOR | 주문 상세 조회 요청에서 주문 소유자 여부를 검증하지 않아, 다른 사용자가 타인의 주문 정보를 조회할 수 있다. |

## 취약한 부분
주문 내역 목록은 로그인한 사용자의 주문만 보여주지만, 주문 상세 화면은 `orderId` 값만 있으면 바로 조회된다.

서버는 해당 주문이 현재 로그인한 사용자의 주문인지, 또는 관리자 권한이 있는 사용자의 요청인지 검증하지 않는다. 그 결과 다른 사용자의 주문번호를 넣어 요청하면 상품명, 결제금액, 배송주소 등 타인의 주문 상세 정보가 그대로 노출된다.
...
## 취약한 코드 동작 설명
주문 상세 조회는 `orderId`를 요청 파라미터로 받아 `getOrderById(orderId)`를 호출한 뒤 바로 화면에 전달한다.

이 과정에서 현재 로그인한 사용자가 해당 주문의 소유자인지 확인하는 로직이 없으므로, 다른 사용자의 `orderId`를 알고 있거나 추측할 수 있다면 타인의 주문 상세 정보를 그대로 확인할 수 있다.

## 취약한 코드 증적자료

### 1. test001 계정의 주문 내역에서 본인 주문 확인
...
- 주문 흐름과 결제 내역에 대한 프라이버시 침해
- 공격자가 다른 사용자의 주문 정보를 수집하거나 추적할 수 있음

## 대응 방안
- 주문 상세 조회 전에 현재 로그인한 사용자가 해당 주문의 소유자인지 검증한다.
- 관리자가 아닌 경우 타인 주문 조회를 차단한다.
- 존재하지 않는 주문 또는 권한 없는 요청은 주문 목록으로 리다이렉트하고 오류 메시지를 제공한다.
...
## 대응 코드 동작 설명
수정 후에는 주문 상세 조회 전에 현재 로그인한 사용자 정보와 조회 대상 주문의 `userId`를 비교한다.

주문 소유자이거나 관리자 권한을 가진 경우에만 상세 화면을 조회할 수 있고, 그 외 사용자는 주문 목록으로 이동된다. 이로써 단순히 `orderId`만 바꾸어 타인 주문을 확인하는 IDOR 공격이 차단된다.

## 대응 코드 증적자료

### 5. test002 계정에서 타인 주문 상세 접근 차단
`추후 추가 예정`

### 6. test001 계정에서는 자신의 주문 상세 정상 조회
`추후 추가 예정`
