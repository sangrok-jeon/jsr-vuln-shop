...
회원가입, 로그인, 마이페이지, 주문, Q&A, 관리자 기능을 포함한 전형적인 쇼핑몰 흐름을 직접 구현하고, 주요 취약점 시나리오를 선정해 재현한 뒤 대응 코드와 재검증 결과까지 정리했습니다. 
정리한 부분 이외 더 취약한 부분이 있어, 전체 취약점을 모두 작성한 건 아닙니다.
...
- 심각도는 재현 가능성, 필요한 권한 수준, 영향 범위(인증 우회, 권한 상승, 상태 변경, 코드 실행, 정보 노출)를 기준으로 `High`, `Medium`, `Low`로 분류했다.
